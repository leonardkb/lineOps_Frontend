...
export default function AddOperatorModal({ runId, slots, onClose, onOperatorAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const operatorResponse = await fetch(`/api/run/${runId}/operators`, {
        method: "POST",
        headers: {
      ...
      for (const op of validOperations) {
        ...
        const operationResponse = await fetch(`/api/add-operation/${runId}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          {success && (
            <div className="mb-4 p-4 bg-green-50 text-green-700 text-base rounded-lg 
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            {!success && (
              <button
                ...
                className="px-6 py-2 text-sm bg-gray-900 text-white rounded-xl
                 hover:bg-gray-800 disabled:opacity-50 flex items-center gap-2"
              >
                {loading ? (
                  <>
