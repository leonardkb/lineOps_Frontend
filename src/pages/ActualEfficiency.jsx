// ActualEfficiency.jsx - Skyrina Efficiency Dashboard with line performance focus
// Shows efficiency metrics, line performance chart, style performance by efficiency, and line performance table
// UPDATED: Uses SAM-based efficiency for all components
...
// Helper to format percentage (show 1 decimal place)
const formatPercent = (value) => {
  if (value == null || isNaN(value)) return '0%';
  return value.toFixed(1) + '%';
};

// Helper to calculate finished garments (from packing operations)
...
// Custom Tooltip for Bar Chart with click instruction
    ...
    return (
      <div className="bg-white rounded-lg shadow-xl border border-gray-200 p-2 sm:p-4">
        ...
        <div className="mt-2 pt-2 border-t border-gray-100">
          <p className="text-xs text-gray-500">Produced: {formatNumber(data.quantity)}</p>
          <p className="text-xs text-gray-500">Target: {formatNumber(data.target)}</p>
        </div>
...
// KPI Card Component
const KPICard = ({ title, value, subtitle, loading, efficiencyColor, suffix = '' }) => (
      ...
      <>
        <div className="flex items-baseline gap-2 sm:gap-3 flex-wrap">
          <span className={`text-2xl sm:text-4xl font-bold ${efficiencyColor || 'text-gray-900'}`}>
            {title.includes('Efficiency') ? formatPercent(value) : formatNumber(value)}{suffix}
          </span>
        </div>
...
// Line Table Row Component
const LineTableRow = ({ line, rank }) => {
  // Use SAM-based efficiency from server
  const efficiency = line.efficiency || 0;
  
  const getEfficiencyColor = (eff) => {
    if (eff >= 90) return 'text-dark-green-600 bg-dark-green-50';
    if (eff >= 80) return 'text-green-600 bg-green-50';
    if (eff >= 70) return 'text-lime-600 bg-lime-50';
    if (eff >= 60) return 'text-orange-600 bg-orange-50';
    return 'text-red-600 bg-red-50';
  };

  const getProgressBarColor = (eff) => {
    if (eff >= 90) return 'bg-dark-green-500';
    if (eff >= 80) return 'bg-green-500';
    if (eff >= 70) return 'bg-lime-500';
    if (eff >= 60) return 'bg-orange-500';
    return 'bg-red-500';
  };

  const getRowBackground = (eff) => {
    if (eff >= 90) return 'bg-dark-green-50/30 hover:bg-dark-green-50/50';
    if (eff >= 80) return 'bg-green-50/30 hover:bg-green-50/50';
    if (eff >= 70) return 'bg-lime-50/30 hover:bg-lime-50/50';
    if (eff >= 60) return 'bg-orange-50/30 hover:bg-orange-50/50';
    return 'bg-red-50/30 hover:bg-red-50/50';
  };

  return (
    <tr className={`border-b border-gray-100 ${getRowBackground(efficiency)} transition-colors`}>
      <td className="px-2 sm:px-4 py-2 sm:py-3">
        <span className="text-xs sm:text-sm text-gray-500 font-medium">{rank}</span>
      </td>
      <td className="px-2 sm:px-4 py-2 sm:py-3">
        <span className="text-xs sm:text-sm font-semibold text-gray-800">{line.lineNo}</span>
      </td>
      <td className="px-2 sm:px-4 py-2 sm:py-3">
      ...
      <td className="px-2 sm:px-4 py-2 sm:py-3">
        <div className="flex items-center gap-1 sm:gap-2">
          <div className="flex-1 h-1.5 sm:h-2 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className={`h-full ${getProgressBarColor(efficiency)} transition-all duration-500 rounded-full`}
              style={{ width: `${Math.min(efficiency, 100)}%` }}
            />
          </div>
          <span className={`text-xs font-semibold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full ${getEfficiencyColor(efficiency)} min-w-[50px] sm:min-w-[60px] text-center`}>
...
const MobileLineCard = ({ line, rank }) => {
  ...
  const getCardBackground = (eff) => {
    if (eff >= 90) return 'bg-dark-green-50';
    if (eff >= 80) return 'bg-green-50';
    if (eff >= 70) return 'bg-lime-50';
    if (eff >= 60) return 'bg-orange-50';
    return 'bg-red-50';
  };
  ...
  const getBorderColor = (eff) => {
    if (eff >= 90) return 'border-dark-green-500';
    if (eff >= 80) return 'border-green-500';
    if (eff >= 70) return 'border-lime-500';
    if (eff >= 60) return 'border-orange-500';
    return 'border-red-500';
  };

  return (
    <div className={`${getCardBackground(efficiency)} rounded-lg p-3 mb-2 border-l-4 ${getBorderColor(efficiency)} transition-all`}>
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500">#{rank}</span>
          <span className="text-sm font-semibold text-gray-800">Line {line.lineNo}</span>
          {efficiency < 70 && (
            <span className="text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded-full">Alert</span>
          )}
        </div>
      ...
      {efficiency < 70 && (
        <div className="mt-2 text-xs">
          <span className="text-red-600">⚠️ Below target efficiency. Requires attention.</span>
        </div>
      )}
    </div>
  );
...
// Line Details Daily Row Component
const LineDetailsRow = ({ run, idx }) => {
  ...
  const getEfficiencyColorClass = (eff) => {
    if (eff >= 90) return 'text-dark-green-600';
    if (eff >= 80) return 'text-green-600';
    if (eff >= 70) return 'text-lime-600';
    if (eff >= 60) return 'text-orange-600';
...
export default function ActualEfficiency() {
  ...
  const [efficiencyData, setEfficiencyData] = useState({
    ...
    linesBelow70: 0,
    lineChartData: [],
    allLines: [],           // All lines for the table
    stylePerformance: []    // Style performance by efficiency
  });
  ...
  useEffect(() => {
    ...
    axios.get(`/api/me`, {
      ...
      .then(res => {
        ...
        if (user.role !== 'skyrina' && user.role !== 'engineer' && user.role !== 'supervisor'&& user.role !== 'master') {
          navigate('/', { replace: true });
          return;
        }
        setUser(user);
  ...
  useEffect(() => {
    if (user) {
      fetchEfficiencyData();
      // Clear line details when filters change
  ...
  const fetchLineDetails = async (lineNo, startDate, endDate, headers) => {
    try {
      ...
      while (currentDate <= endDate) {
        ...
        try {
          const runsRes = await axios.get(`/api/line-runs/${lineNo}`, { headers });
  ...
  // Clear line filter / close details
  ...
  const fetchEfficiencyData = async () => {
    ...
    try {
      ...
      // Fetch line efficiency data from the dedicated endpoint
      let url = `/api/skyrina/line-efficiency?startDate=${startStr}&endDate=${endStr}`;
      if (selectedStyle !== 'all') url += `&style=${encodeURIComponent(selectedStyle)}`;
      if (selectedLine !== 'all') url += `&lineNo=${selectedLine}`;
      
      const efficiencyRes = await axios.get(url, { headers });
      
      // Fetch period summary for CORRECT global efficiency calculation
      let summaryUrl = `/api/skyrina/period-summary?startDate=${startStr}&endDate=${endStr}`;
      if (selectedStyle !== 'all') summaryUrl += `&style=${encodeURIComponent(selectedStyle)}`;
      if (selectedLine !== 'all') summaryUrl += `&lineNo=${selectedLine}`;
      
      const summaryRes = await axios.get(summaryUrl, { headers });
      
      // Fetch style performance data - USES efficiency field (SAM-based)
      let styleUrl = `/api/skyrina/style-performance?startDate=${startStr}&endDate=${endStr}`;
      if (selectedStyle !== 'all') styleUrl += `&style=${encodeURIComponent(selectedStyle)}`;
      if (selectedLine !== 'all') styleUrl += `&lineNo=${selectedLine}`;
      const styleRes = await axios.get(styleUrl, { headers });
      
      // Fetch line performance detail for the table - USES efficiency field (SAM-based)
      let lineDetailUrl = `/api/skyrina/line-performance-detail?startDate=${startStr}&endDate=${endStr}`;
      if (selectedStyle !== 'all') lineDetailUrl += `&style=${encodeURIComponent(selectedStyle)}`;
      if (selectedLine !== 'all') lineDetailUrl += `&lineNo=${selectedLine}`;
      const lineDetailRes = await axios.get(lineDetailUrl, { headers });
      
      // Fetch available styles and lines for filters
      const [stylesRes, linesRes] = await Promise.all([
        axios.get(`/api/skyrina/available-styles?startDate=${startStr}&endDate=${endStr}`, { headers }),
        axios.get(`/api/skyrina/available-lines?startDate=${startStr}&endDate=${endStr}`, { headers })
      ]);
      
      if (stylesRes.data.success) {
        setAvailableStyles(['all', ...stylesRes.data.styles]);
      }
      
      if (linesRes.data.success) {
        setAvailableLines(['all', ...linesRes.data.lines]);
      }
      ...
      let lineChartData = [];
      let allLines = [];
      let stylePerformance = [];
      let linesAbove70 = 0;
      let linesBelow70 = 0;
      
      // Get summary data
      if (summaryRes.data.success && summaryRes.data.summary) {
        totalTarget = summaryRes.data.summary.totalTarget || 0;
        totalProduced = summaryRes.data.summary.totalSewed || 0;
        globalEfficiency = summaryRes.data.summary.avgEfficiency || 0;
      }
      
      // Get chart data - uses efficiency from server
      if (efficiencyRes.data.success && efficiencyRes.data.lines) {
        lineChartData = efficiencyRes.data.lines;
        
        // Calculate above/below 70 counts from lineChartData
        linesAbove70 = lineChartData.filter(line => (line.efficiency || 0) >= 70).length;
        linesBelow70 = lineChartData.filter(line => (line.efficiency || 0) < 70).length;
      }
      
      // Get line performance detail for the table - USE efficiency field (SAM-based)
      if (lineDetailRes.data.success && lineDetailRes.data.lines) {
        allLines = lineDetailRes.data.lines.map(line => ({
          style: line.style,
          lineNo: line.lineNo,
          target: line.target,
          produced: line.produced,
          efficiency: line.efficiency || 0,  // SAM-based efficiency from server
          compliance: line.compliance || 0   // Production compliance (for reference)
        })).sort((a, b) => b.efficiency - a.efficiency);
      }
      
      // Get style performance - USE efficiency field (SAM-based)
      if (styleRes.data.success && styleRes.data.styles) {
        stylePerformance = styleRes.data.styles
          .sort((a, b) => b.efficiency - a.efficiency)  // Sort by SAM-based efficiency
          .slice(0, 15);
      }
      ...
      setEfficiencyData({
        ...
        linesAbove70: linesAbove70,
        linesBelow70: linesBelow70,
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-br from-slate-50 to-gray-100">
        <div className="text-center px-4">
          ...
          <p className="mt-4 text-gray-600 font-medium">Loading Efficiency Dashboard...</p>
        </div>
      </div>
    );
  }

  // Filter chart data based on selected line details
  const filteredChartData = showLineDetails && selectedLineDetails
    ? efficiencyData.lineChartData.filter(line => String(line.lineNo) === String(selectedLineDetails))
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <div className="mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Efficiency Analysis - {getPeriodTitle()}
          </h2>
        </div>

        {/* KPI Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-5 mb-6 sm:mb-8">
          <KPICard 
            title="Total Target"
            value={efficiencyData.totalTarget}
            subtitle="target pieces"
            loading={loading}
          />
          <KPICard 
            title="Total Produced"
            value={efficiencyData.totalProduced}
            subtitle="actual pieces"
            loading={loading}
          />
          <KPICard 
            title="Global Efficiency"
            value={efficiencyData.globalEfficiency}
            subtitle="weighted average"
            loading={loading}
            efficiencyColor={getEfficiencyColorClass(efficiencyData.globalEfficiency)}
          />
          <KPICard 
            title="Lines ≥70%"
            value={efficiencyData.linesAbove70}
            subtitle="meeting or exceeding target"
            loading={loading}
            efficiencyColor="text-green-600"
          />
          <KPICard 
            title="Lines <70%"
            value={efficiencyData.linesBelow70}
        ...
        {/* Line Efficiency Performance Chart */}
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8 border border-gray-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4">
            <h3 className="text-base sm:text-lg font-bold text-gray-900">Line Efficiency Performance</h3>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 sm:py-16 text-gray-400">
              <p>No line efficiency data available for the selected period</p>
            </div>
          )}
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {lineDetailsData.runs.map((run, idx) => (
                    <LineDetailsRow key={idx} run={run} idx={idx} />
                  ))}
                </tbody>
              </table>
            </div>
        ...
        {/* Two Column Layout - Style Performance (Left) and Line Performance Table (Right) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {/* Left Side: Style Performance by Efficiency (Horizontal Bar Chart) */}
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 border border-gray-100">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-4 sm:mb-6">Style Performance (Top 15 by Efficiency)</h3>
            ...
            {loading ? (
            ...
            ) : efficiencyData.stylePerformance.length > 0 ? (
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1 sm:pr-2">
                {efficiencyData.stylePerformance.map((style, index) => {
                  // USE efficiency field (SAM-based) from server
                  const efficiency = style.efficiency || 0;
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-6 sm:h-8 bg-gray-100 rounded-lg overflow-hidden">
                            <div 
                              className="h-full rounded-lg transition-all duration-500 flex items-center justify-end px-1 sm:px-2"
                              style={{ width: `${Math.min(efficiency, 100)}%`, backgroundColor: barColor }}
                            >
                          ...
                          <span className={`text-xs font-semibold min-w-[50px] text-right ${getEfficiencyColorClass(efficiency)}`}>
                            {formatPercent(efficiency)}
                          </span>
                        </div>
                      </div>
              ...
              <div className="text-center py-8 sm:py-12 text-gray-400">
                <p>No style performance data available</p>
              </div>
            )}
            ...
            <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-gray-400">
              <span>Showing top {Math.min(efficiencyData.stylePerformance.length, 15)} styles by efficiency</span>
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 border border-gray-100">
            ...
            ) : efficiencyData.allLines.length > 0 ? (
              <>
                <div className="hidden md:block overflow-x-auto max-h-[500px] overflow-y-auto">
                  <table className="w-full">
                    <thead className="sticky top-0 bg-white z-10">
                      <tr className="border-b-2 border-gray-200">
                        <th className="px-2 sm:px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-10">#</th>
                        <th className="px-2 sm:px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Line</th>
                        <th className="px-2 sm:px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Style</th>
                        <th className="px-2 sm:px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Target</th>
                        <th className="px-2 sm:px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Prod.</th>
                        <th className="px-2 sm:px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider min-w-[100px]">Efficiency</th>
                      </tr>
                    </thead>
                    <tbody>
                      {efficiencyData.allLines.map((line, idx) => (
                        <LineTableRow key={`line-${line.lineNo}`} line={line} rank={idx + 1} />
                      ))}
                    </tbody>
                  </table>
                </div>
                ...
                <div className="md:hidden max-h-[500px] overflow-y-auto">
                  {efficiencyData.allLines.map((line, idx) => (
                    <MobileLineCard key={`line-${line.lineNo}`} line={line} rank={idx + 1} />
                  ))}
                </div>
              </>
            ) : (
            ...
            <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-gray-400">
              <span>Total lines: {efficiencyData.allLines.length}</span>
              <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
                <span className="flex items-center gap-1">
      ...
      <footer className="mt-auto py-4 sm:py-6 bg-white/80 backdrop-blur-sm border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 text-center">
          <p className="text-xs sm:text-sm text-gray-500">
            Skyrina • Efficiency Dashboard • {new Date().toLocaleDateString('en-US')}
          </p>
        </div>
      </footer>
    </div>
